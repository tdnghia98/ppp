...
-- beneficiary 2 is the sender
mkValidator :: VestingDatum -> () -> ScriptContext -> Bool
mkValidator (VestingDatum b1 b2 vestingDeadline) _ scriptContext
  | checkDeadline == True = traceIfFalse "Not beneficiary 2" (checkSig b2)
  | otherwise = traceIfFalse "Not beneficiary 1" (checkSig b1)
  where
    -- FIX ME!
    info :: TxInfo
    ...
    checkDeadline :: Bool
    checkDeadline = from vestingDeadline `contains` txInfoValidRange info
